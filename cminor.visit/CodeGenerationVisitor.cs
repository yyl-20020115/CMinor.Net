...
public class CodeGenerationVisitor : Visitor
{
    ...
    public override void Visit(Program n)
    {
        ...
        foreach (var ss in n.StringSymbols)
        {
            output.WriteLine((ss.Label) + (": .asciz \"") + (StringLiteral.Escape(ss.Value))
                + ("\"")
                );
        }
        foreach (var gv in n.GlobalVariables)
        {
            gv.Accept(this);
        }
    ...
    public override void visit(PrintStatement n)
    {
        ...
        for (int i = num - 1; i >= 0; i += -1)
        {
            ...
            if (expression.Type == Types.boolean_type)
            {
